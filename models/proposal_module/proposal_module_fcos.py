...
from models.proposal_module.ROI_heads.roi_heads import StandardROIHeads, StandardROIHeads_qa
...
class ProposalModule(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling,
        ...
        if not qa:
            ...
            self.proposal = StandardROIHeads_qa(num_heading_bin=num_heading_bin, num_class=num_class,
                                                num_size_cluster=num_size_cluster, seed_feat_dim=256)

        self.proposal_proj = nn.Linear(128, 128)
    ...
    def forward(self, xyz, features, data_dict):
        ...
        data_dict["proposal_proj_emb"] = F.normalize(self.proposal_proj(data_dict['aggregated_vote_features']), dim=-1)

        # --------- PROPOSAL GENERATION ---------
        if not self.qa:
            data_dict = self.proposal(features, data_dict)
        else:
            data_dict = self.proposal(features, data_dict, self.mean_size_arr)
