...
class MatchModule(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # cross-attention
        feature1, cross_att = self.grounding_cross_attn(feature1, lang_fea, lang_fea, data_dict["attention_mask"], output_attn=True)
        # feature1 = self.grounding_cross_attn(proposal_fea, lang_fea, lang_fea, data_dict["attention_mask"])
        data_dict['cross_att'].append(cross_att.detach().cpu())

        for n in range(self.depth):
            feature1 = self.self_att[n](feature1, feature1, feature1)
            feature1 = self.cross_att[n](feature1, lang_fea, lang_fea, data_dict["attention_mask"])
