...
from lib.joint.dataset_qa import ScannetQADataset, ScannetQADatasetConfig
from lib.joint.solver_qa import Solver
from lib.configs.config_qa import CONF
from models.network.network_ft_qa import JointNet as ScanQA
...
def parse_option():
    ...
    parser.add_argument("--lr", type=float, help="initial learning rate", default=0.0001)  # 0.0005
    ...
    parser.add_argument("--vote_radius", type=float, help="", default=0.3)
    parser.add_argument("--vote_nsample", type=int, help="", default=16)
    parser.add_argument("--pointnet_width", type=int, help="", default=1)
    parser.add_argument("--pointnet_depth", type=int, help="", default=2)
    parser.add_argument("--seed_feat_dim", type=int, help="", default=256)
    ...
    parser.add_argument("--mcan_flat_mlp_size", type=int, help="", default=256)
    parser.add_argument("--mcan_flat_glimpses", type=int, help="", default=1)
    parser.add_argument("--mcan_flat_out_size", type=int, help="", default=512)
    parser.add_argument("--mcan_num_heads", type=int, help="", default=8)
    parser.add_argument("--mcan_num_layers", type=int, help="", default=2)

    parser.add_argument("--use_unanswerable", action="store_true", help="")  ##########
...
def get_dataloader(args, scanqa, all_scene_list, split, config, augment):
    ...
    dataset = ScannetQADataset(
        ...
        use_unanswerable=args.use_unanswerable,  #
...
def get_model(args, config):
    ...
    if args.use_pretrained:
        ...
        # model.lang = pretrained_model.lang  # NO_PRE_LANG
        model.relation = pretrained_model.relation
        model.temp = pretrained_model.temp
...
def get_solver(args, dataloader):
    ...
    # weight_dict = {
    #     'fusion_backbone': {'lr': 0.0005},
    #     'answer_cls': {'lr': 0.0005}
    # }
