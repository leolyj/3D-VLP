...
def get_model(args, dataset, device, root=CONF.PATH.OUTPUT, eval_pretrained=False):
    ...
    if eval_pretrained:
        ...
        # load
        model_name = "model_last.pth" if args.use_last else "caption_model.pth"
...
def eval_caption(args):
    ...
    log_fout.flush()
    info_str = "[BLEU-1] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(bleu[0][0], max(bleu[1][0]), min(bleu[1][0]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[BLEU-2] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(bleu[0][1], max(bleu[1][1]), min(bleu[1][1]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[BLEU-3] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(bleu[0][2], max(bleu[1][2]), min(bleu[1][2]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[BLEU-4] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(bleu[0][3], max(bleu[1][3]), min(bleu[1][3]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[CIDEr] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(cider[0], max(cider[1]), min(cider[1]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[ROUGE-L] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(rouge[0], max(rouge[1]), min(rouge[1]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
    info_str = "[METEOR] Mean: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(meteor[0], max(meteor[1]), min(meteor[1]))
    log_fout.write(info_str + "\n")
    log_fout.flush()
...
def eval_detection(args):
    ...
    for data in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            data = get_joint_loss(data, device, DC, weights=dataset.weights, detection=True, caption=False, reference=False)
