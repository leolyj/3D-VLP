...
from lib.loss_helper.loss_pretrain_ft import get_pretrain_loss
...
[loss] train_mpm_loss: {train_mpm_loss}
...
[train] train_mpm_loss: {train_mpm_loss}
...
[val]   val_mpm_loss: {val_mpm_loss}
[val]   val_mlm_loss: {val_mlm_loss}
[val]   val_align_loss: {val_align_loss}
...
class Solver():
    def __init__(self, model, device, config, dataset, dataloader, optimizer, stamp, val_step=10, num_epoch=150,
                 detection=True, reference=True, use_lang_classifier=True, orientation=False,
                 distance=False, use_tf=True, lr_decay_step=None, lr_decay_rate=None, bn_decay_step=None, bn_decay_rate=None,
                 criterion="sum", checkpoint_best=None,
                 ):
        ...
        # tensorboard
        os.makedirs(os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "tensorboard/train"), exist_ok=True)
        os.makedirs(os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "tensorboard/val"), exist_ok=True)
        self._log_writer = {
            "train": SummaryWriter(os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "tensorboard/train")),
            "val": SummaryWriter(os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "tensorboard/val"))
        }

        # training log
        log_path = os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "log.txt")
        self.log_fout = open(log_path, "a")

        eval_path = os.path.join(CONF.PATH.OUTPUT_PRETRAIN, stamp, "eval.txt")
    ...
    def __call__(self, epoch, verbose):
        ...
        for epoch_id in range(epoch):
            try:
                ...
                model_root = os.path.join(CONF.PATH.OUTPUT_PRETRAIN, self.stamp)
    ...
    def _dump_log(self, phase, is_eval=False):
        if phase == "train" and not is_eval:
            log = {
                "loss": ["loss", "ref_loss", "mpm_loss", "mlm_loss", "align_loss", "lang_loss", "ori_loss", "dist_loss", "objectness_loss",
                         "vote_loss", "box_loss"],
                "score": ["lang_acc", "ref_acc", "ori_acc", "obj_acc", "pos_ratio", "neg_ratio",
                          "iou_rate_0.25", "iou_rate_0.5", "max_iou_rate_0.25", "max_iou_rate_0.5"]
            }
            for key in log:
                for item in log[key]:
    ...
    def _compute_loss(self, data_dict):
        ...
        self._running_log["mpm_loss"] = data_dict["mpm_loss"]
    ...
    def _feed(self, dataloader, phase, epoch_id, is_eval=False):
        ...
        if not is_eval:
            ...
            for data_dict in dataloader:
                ...
                self.log[phase]["mpm_loss"].append(self._running_log["mpm_loss"].item())
                self.log[phase]["mlm_loss"].append(self._running_log["mlm_loss"].item())
                self.log[phase]["align_loss"].append(self._running_log["align_loss"].item())
            ...
            if phase == "val" and cur_criterion == "sum":
                ...
                cur_best = ground_cur_best
                for m in ground_metrics:
                    print(m)
                    print('************', np.mean(self.log[phase][m]))
            else:
                ground_cur_best = 0.
                cur_best = 0.
    ...
    def _finish(self, epoch_id):
        ...
        checkpoint_root = os.path.join(CONF.PATH.OUTPUT_PRETRAIN, self.stamp)
        ...
        for phase in ["train", "val"]:
            self._log_writer[phase].export_scalars_to_json(
                os.path.join(CONF.PATH.OUTPUT_PRETRAIN, self.stamp, "tensorboard/{}".format(phase), "all_scalars.json"))
    ...
    def _train_report(self, epoch_id):
        ...
        iter_report = self.__iter_report_template.format(
            ...
            train_mpm_loss=round(np.mean([v for v in self.log["train"]["mpm_loss"]]), 5),
            train_mlm_loss=round(np.mean([v for v in self.log["train"]["mlm_loss"]]), 5),
            train_align_loss=round(np.mean([v for v in self.log["train"]["align_loss"]]), 5),
    ...
    def _epoch_report(self, epoch_id):
        ...
        epoch_report = self.__epoch_report_template.format(
            ...
            val_mpm_loss=round(np.mean([v for v in self.log["val"]["mpm_loss"]]), 5),
            val_mlm_loss=round(np.mean([v for v in self.log["val"]["mlm_loss"]]), 5),
            val_align_loss=round(np.mean([v for v in self.log["val"]["align_loss"]]), 5),
    ...
    def _best_report(self):
        ...
        with open(os.path.join(CONF.PATH.OUTPUT_PRETRAIN, self.stamp, "best.txt"), "w") as f:
            f.write(best_report)
