...
def eval_cap(model, device, dataset, dataloader, phase, folder, 
    ...
    if no_caption:
        ...
        #     value = [val.split('.')[0] + '. eos' for val in candidates[key]]
        #     candidates[key] = value
        # for key in corpus.keys():
        #     value = [val.split('.')[0] + '. eos' for val in corpus[key]]
        #     corpus[key] = value
        # for key in candidates.keys():
        #     value = ['sos '+' . '.join(val.split('.')[1:]) for val in candidates[key]]
        #     candidates[key] = value
        # for key in corpus.keys():
        #     value = ['sos '+' . '.join(val.split('.')[1:]) for val in corpus[key]]
        #     corpus[key] = value
