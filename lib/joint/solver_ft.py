...
class Solver():
    def __init__(self, model, device, config, dataset, dataloader, optimizer, stamp, val_step=10, num_ground_epoch=50,
                 ...
                 detection=True, reference=True, use_lang_classifier=True, caption=True, orientation=False,
                 distance=False, use_tf=True,
    ...
    def _dump_log(self, phase, is_eval=False):
        if phase == "train" and not is_eval:
            log = {
                "loss": ["loss", "ref_loss", "align_loss", "lang_loss", "cap_loss", "ori_loss", "dist_loss", "objectness_loss",
                         "vote_loss", "box_loss"],
                "score": ["lang_acc", "ref_acc", "cap_acc", "ori_acc", "obj_acc", "pred_ious", "pos_ratio", "neg_ratio",
    ...
    def _compute_loss(self, data_dict):
        data_dict = get_ft_loss(
            ...
            num_pretrain_epoch=self.num_pretrain_epoch
        )
    ...
    def _eval(self, phase, epoch):
        if self.caption and phase != "train" and epoch >= self.num_ground_epoch + self.num_pretrain_epoch:
            ...
            bleu_, cider_, rouge_, meteor_ = eval_cap(
                model=self.model,
                device=self.device,
                dataset=self.dataset["eval"]["val_scene"],
                dataloader=self.dataloader["eval"]["val_scene"],
            ...
            print('*cider*', cider[0])
            print('*rouge*', rouge[0])
            print('*meteor*', meteor[0])
            print('------------------')
            print('cap 0.25 iou')
            print('*bleu*', bleu_[0])
            print('*cider*', cider_[0])
            print('*rouge*', rouge_[0])
            print('*meteor*', meteor_[0])

            info_str = "0.25 b{} c{} r{} m{}".format(bleu_[0], cider_[0], rouge_[0], meteor_[0])
            self._log_eval(info_str)
            info_str = "0.5 b{} c{} r{} m{}".format(bleu[0], cider[0], rouge[0], meteor[0])
            self._log_eval(info_str)
        ...
        if not is_eval:
            ...
            for data_dict in dataloader:
                ...
                if phase == "train" and not is_eval:
                    ...
                    # evaluation
                    if epoch_id >= self.num_ground_epoch + self.num_pretrain_epoch:
