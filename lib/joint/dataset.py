...
class ReferenceDataset(Dataset):
    ...
    def _ground_tranform_des_(self):
        with open(GLOVE_PICKLE, "rb") as f:
            glove = pickle.load(f)
    ...
    def _load_data(self, dataset_name):
        ...
        self.ground_lang, self.ground_lang_ids, self.ground_lang_main = self._ground_tranform_des_()
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.lang_num_max):
            if i < lang_num:
                ...
                token_i = ' '.join(self.scanrefer_new[idx][i]["token"])
        ...
        id2class = DC.nyu40id2class
        id2class[0] = 18
        id2class[1] = 18
        id2class[2] = 18
        id2class[22] = 18
        data_dict["sem_labels"] = np.array([id2class[int(k)] for k in semantic_labels]).astype(np.int32)

        # data_dict["tokens_list"] = tokens_list
