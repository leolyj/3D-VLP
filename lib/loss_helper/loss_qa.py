...
from lib.ap_helper.ap_helper_votenet import parse_predictions
from lib.loss_helper.loss import SoftmaxRankingLoss
# from lib.loss_helper.loss_detection import compute_vote_loss, compute_objectness_loss, compute_box_loss
...
def compute_box_and_sem_cls_loss(data_dict, config):
    ...
    # Compute center loss
    pred_center = data_dict['pred_center']
    gt_center = data_dict['center_label'][:,:,0:3]
...
def compute_reference_loss(data_dict, config):
    ...
    pred_center = data_dict['pred_center'].detach().cpu().numpy() # (B,K,3)
    ...
    # ------------------ for alignment ------------------
    data_dict['align_labels'] = cluster_labels

    return loss_ref, cluster_preds, cluster_labels


def compute_align_loss(data_dict):
    align_label = data_dict['align_labels']
    temp = data_dict["temp"]
    bbox_feature = data_dict['bbox_proj_emb']  # (B, num_proposal, 128)
    lang_emb = data_dict['lang_proj_emb']  # (80, 128)

    batch_size, n_proposal = bbox_feature.shape[:2]
    ...
    align_label = align_label.transpose(0, 1)

    sim_targets_bl = torch.zeros(sim_bbox_lang.size()).to(sim_bbox_lang.device)
    for i in range(batch_size):
        sim_targets_bl[i*n_proposal:(i+1)*n_proposal, i:(i+1)] = align_label[:, i].unsqueeze(1)
...
def get_loss(data_dict, config, detection=True, use_reference=True, use_lang_classifier=False, use_answer=True, loss_weights=None):
    ...
    # if use_answer and data_dict["epoch"] > 9:  # 10/15 pre10
    #     data_dict["answer_loss"] = compute_answer_classification_loss(data_dict)
    # else:
    ...
    loss_align = compute_align_loss(data_dict)
    data_dict['align_loss'] = 10 * loss_align

    loss += data_dict['align_loss']
